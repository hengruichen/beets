...
class ListenBrainzPlugin(BeetsPlugin):
    ...
    def get_tracks_from_listens(self, listens):
        ...
        for track in listens:
            ...
            tracks.append({"mbid": mbid, "title": track["track_metadata"]["title"]})

        return tracks

    def get_mb_recording_id(self, track):
        """Returns the MusicBrainz ID of a track."""
        title = track["track_metadata"]["title"]
        release = track["track_metadata"]["release_name"]
        params = {"query": f"{title} {release}"}
        response = self._make_request(
            f"{self.ROOT}/search/recordings", params=params
        )
        if response is not None:
            recordings = response["payload"]["recordings"]
            if recordings:
                return recordings[0]["recording_mbid"]
        return None
