...
class ListenBrainzPlugin(BeetsPlugin):
    ...
    def get_mb_recording_id(self, track):
        """Returns the MBID for a track."""
        search_url = "https://musicbrainz.org/ws/2/recording/?query="
        search_url += f'track:"{track["track_metadata"]["track_name"]}"'
        search_url += "&fmt=json&limit=1"
        resp = requests.get(search_url)
        resp.raise_for_status()
        resp_json = resp.json()
        if resp_json["recording-list"]:
            return resp_json["recording-list"][0]["id"]
        else:
            return None
